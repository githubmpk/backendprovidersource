...
IF OBJECT_ID('dbo.udfRSR_Mic_Get_Comments') IS NOT NULL
BEGIN
	DROP FUNCTION dbo.udfRSR_Mic_Get_Comments
	IF OBJECT_ID('dbo.udfRSR_Mic_Get_Comments') IS NOT NULL PRINT 'Failed to drop sp!!!' ELSE PRINT 'Dropped'
END
GO

/****** Object:  UserDefinedFunction [dbo].[udfRSR_Mic_Get_Comments]    Script Date: 5/22/2019 10:39:21 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE FUNCTION [dbo].[udfRSR_Mic_Get_Comments] (@SpcID BIGINT)
RETURNS
...
,COALESCE([RSReporting].[dbo].[udfRSR_Get_MIC_Result_Comments] (R.[biId],1),'')
...
AND Z.[Prompt]=Org.[vchPromptMnemonic]
  ...
  AND COALESCE([RSReporting].[dbo].[udfRSR_Get_MIC_Result_Comments] (R.[biId],1),'')<>''
...
,COALESCE([RSReporting].[dbo].[udfRSR_Get_MIC_Result_Comments] (R.[biId],2),'')
...
on Org.iId=Mic.iOrganismId
  ...
  WHERE R.[biSpecimenId]=@SpcID
  AND COALESCE([RSReporting].[dbo].[udfRSR_Get_MIC_Result_Comments] (R.[biId],2),'')<>''
RETURN
END
